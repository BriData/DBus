...
import static com.alibaba.otter.canal.protocol.CanalEntry.Pair.newBuilder;
...
public class LogEventConvert extends AbstractCanalLifeCycle implements BinlogParser<LogEvent> {
    ...
    private Entry parseRowsEvent(RowsLogEvent event) {
        ...
        try {
            ...
            Header.Builder hBuilder = createHeaderBuilder(binlogFileName,
                    event.getHeader(),
                    table.getDbName(),
                    table.getTableName(),
                    eventType);
            ...
            Pair pair = createSpecialPair("rowCount", String.valueOf(rowCount));
            hBuilder.addProps(pair);
            Header header = hBuilder.build();
                ...
                switch (javaType) {
                    ...
                    case Types.LONGVARBINARY:
                        ...
                        if (fieldMeta != null && isText(fieldMeta.getColumnType())) {
                            if (isShortText(fieldMeta.getColumnType())) {
                                columnBuilder.setValue(new String((byte[]) value, charset));
                            } else {
                                //set Long TEXT as ""
                                columnBuilder.setValue(new String((byte[])"".getBytes(), charset));
                            }
                            javaType = Types.CLOB;
                        } else {
                            // byte数组，直接使用iso-8859-1保留对应编码，浪费内存
                            //System.out.println("TYPE:"+ fieldMeta.getColumnType() +
                            //        ",value=" + new String((byte[]) value, ISO_8859_1));

                            if (isLargeBlob(fieldMeta.getColumnType())) {
                                //set Long TEXT as ""
                                columnBuilder.setValue(new String((byte[]) "".getBytes(), ISO_8859_1));
                            } else {
                                //include tiniblob, blob, binary, varbinary
    ...
    private Header createHeader(String binlogFile, LogHeader logHeader, String schemaName, String tableName,
                                EventType eventType) {
        Header.Builder  headerBuilder = createHeaderBuilder(binlogFile, logHeader, schemaName, tableName, eventType);
    ...
    private boolean isShortText(String columnType) {
        return "TEXT".equalsIgnoreCase(columnType) || "TINYTEXT".equalsIgnoreCase(columnType);
    }

    private boolean isLargeBlob(String columnType) {
        return "MEDIUMBLOB".equalsIgnoreCase(columnType) || "LONGBLOB".equalsIgnoreCase(columnType);
    }
