...
public class TableController extends BaseController {
    ...
    public ResultEntity getDesensitizationInfo(@PathVariable Integer tableId) {
        try {
            List<EncodeColumns> result = tableService.getDesensitizationInfo(tableId);
            return resultEntityBuilder().payload(result).build();
        } catch (Exception e) {
    ...
    @GetMapping("/find")
    public ResultEntity getTables(Integer dsId, Integer schemaId, String schemaName, String tableName,
                                  @RequestParam Integer pageNum,
                                  @RequestParam Integer pageSize) {
        return resultEntityBuilder().payload(tableService.getTablesInPages(dsId, schemaId, schemaName, tableName, pageNum, pageSize)).build();
    }
    ...
    public ResultEntity reInitTableMeta(@PathVariable Integer tableId) {
        try {
            return resultEntityBuilder().payload(tableService.reInitTableMeta(tableId)).build();
        } catch (Exception e) {
    ...
    @GetMapping("/findBySchemaIdAndTableName")
    public ResultEntity findBySchemaIdAndTableName(@RequestParam Long schemaId, @RequestParam String tableName) {
        try {
            return resultEntityBuilder().payload(tableService.findBySchemaIdAndTableName(schemaId.intValue(), tableName)).build();
        } catch (Exception e) {
            logger.error("Exception encountered while request findBySchemaIdAndTableName.", e);
            return resultEntityBuilder().status(MessageCode.EXCEPTION).build();
        }
    }

    @GetMapping("/getTableRows/{id}")
    public ResultEntity getTableRows(@PathVariable Integer id) {
        try {
            return resultEntityBuilder().payload(tableService.getTableRows(id)).build();
        } catch (Exception e) {
    ...
    public ResultEntity moveSourceTables(@RequestBody Map<String, Object> param) {
        try {
            return resultEntityBuilder().status(tableService.moveSourceTables(param)).build();
        } catch (Exception e) {
    ...
    @GetMapping("/getTablesByDsId/{dsId}")
    public ResultEntity getTablesByDsId(@PathVariable Integer dsId) {
        return resultEntityBuilder().payload(tableService.getTablesByDsId(dsId)).build();
    }

    @PostMapping("/batchDeleteTableByTableIds")
    public ResultEntity moveSourceTables(@RequestBody List<Integer> tableIds) {
        try {
            return tableService.batchDeleteTableByTableIds(tableIds);
        } catch (Exception e) {
            logger.error("Exception encountered while request moveSourceTables.", e);
            return resultEntityBuilder().status(MessageCode.EXCEPTION).build();
        }
