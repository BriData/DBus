...
import {DELETE_SINKER_TABLE_API} from "@/app/containers/SinkManage/api";

export default class SinkerTableGrid extends Component {
  componentWillMount() {
  }
  ...
  handleDelete = record => {
    Request(`${DELETE_SINKER_TABLE_API}/${record.id}`, {
      method: 'get'
    })
      .then(res => {
        if (res && res.status === 0) {
