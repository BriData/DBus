...
public class ToolSetService {
    ...
    public static final String HEARTBEAT_RELOAD = "HEARTBEAT_RELOAD";
    public static final String ROUTER_RELOAD = "ROUTER_RELOAD";
    public static final String SINKER_RELOAD = "SINKER_RELOAD";
    ...
    public JSONObject readZKNode(String type) throws Exception {
        switch (type) {
            ...
            case ROUTER_RELOAD:
                return getZkNodes(Constants.CONTROL_MESSAGE_RESULT_ROOT + "/" + ROUTER_RELOAD);
        }
        return null;
    }
        ...
        if (zkService.isExists(path)) {
            ...
            if (children.size() > 0) {
                for (String childPath : children) {
                    ...
                    JSONObject childJson = JSONObject.parseObject(new String(childData, KeeperConstants.UTF8));
    ...
    public ResultEntity kafkaReader(Map<String, String> map, Integer userId, String userRole) throws Exception {
        ...
        String topic = map.get("topic");
        if (!getTopics(null, null).contains(topic)) {
            resultEntity.setStatus(MessageCode.NO_AUTHORITY_FOR_THIS_TOPIC);
            resultEntity.setMessage("topic不存在!");
            return resultEntity;
        }
    ...
    public void sendCtrlMessageEasy(Integer dsId, String dsName, String dsType) {
        if ("mysql".equals(dsType)) {
            ...
            reloadConfig(dsId, dsName, DISPATCHER_RELOAD_CONFIG);
            reloadConfig(dsId, dsName, APPENDER_RELOAD_CONFIG);
            this.reloadHeartBeat();
            reloadConfig(dsId, dsName, EXTRACTOR_RELOAD_CONF);
        } else {
    ...
    public int reloadSinkerConfig(String topic, String message) {
        HashMap<String, String> map = makeCtrlMessageParam(topic);
    ...
    private void reloadHeartBeat() {
        ...
        try {
            zkService.setData("/DBus/HeartBeat/Control", json.toString().getBytes(KeeperConstants.UTF8));
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
    }
    ...
    public JSONObject checkEnvironment() throws Exception {
        ...
        try {
            JSONArray nimbuses = JSONObject.parseObject(stormTopoHelper.nimbusSummary()).getJSONArray("nimbuses");
        ...
        try {
            //心跳节点状态
            String[] heartbeatList = global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_HEARTBEAT_HOST).split(",");
            int heartbeatport = Integer.parseInt(global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_CLUSTER_SERVER_SSH_PORT));
            String heartbeatuser = global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_CLUSTER_SERVER_SSH_USER);
        ...
        try {
            //kafka节点状态
            String[] bootstrapList = global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_BOOTSTRAP_SERVERS).split(",");
            ...
            for (String bootstrap : bootstrapList) {
                ...
                boolean b = false;
                try {
                    b = configCenterService.urlTest(ipPort[0], Integer.parseInt(ipPort[1]));
                } catch (NumberFormatException e) {
                    logger.error(e.getMessage(), e);
                    b = false;
                }
        ...
        //zk节点状态
        try {
            String zk_url = global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_ZK_STR);
        ...
        try {
            //grafana状态
            String grafanaUrl = global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_GRAFANA_DBUS_URL);
        ...
        try {
            //influxdb状态
            String influxdbUrl = global.getProperty(KeeperConstants.GLOBAL_CONF_KEY_INFLUXDB_DBUS_URL);
    ...
    private void updateMySqlConfig() throws Exception {
        ...
        LinkedHashMap<String, String> linkedHashMap = formatPropertiesString(new String(data, KeeperConstants.UTF8));
        linkedHashMap.put("driverClassName", (String) map.get("spring.datasource.driver-class-name"));
        linkedHashMap.put("url", (String) map.get("spring.datasource.url"));
        linkedHashMap.put("username", (String) map.get("spring.datasource.username"));
        linkedHashMap.put("password", (String) map.get("spring.datasource.password"));
        StringBuilder sb = new StringBuilder();
        for (Map.Entry<String, String> entry : linkedHashMap.entrySet()) {
            sb.append(entry.getKey()).append("=").append(entry.getValue()).append("\n");
        }
        zkService.setData(Constants.MYSQL_PROPERTIES_ROOT, sb.toString().getBytes(KeeperConstants.UTF8));
    }
    ...
    private void initializeDsZkConf(String dsName, Properties globalPro) throws Exception {
        ...
        for (String confFilePath : InitZooKeeperNodesTemplate.ZK_TEMPLATES_NODES_PATHS) {
            ...
            for (int i = 1; i < confFilePathPrefixsCount; i++) {
                ...
                precedantPath = precedantPath.replace(KeeperConstants.DS_NAME_PLACEHOLDER, dsName);
            ...
            fileContent = fileContent.replace(KeeperConstants.BOOTSTRAP_SERVER_PLACEHOLDER, globalPro.getProperty(KeeperConstants.GLOBAL_CONF_KEY_BOOTSTRAP_SERVERS));
            fileContent = fileContent.replace(KeeperConstants.ZK_SERVER_PLACEHOLDER, env.getProperty("zk.str"));
            fileContent = fileContent.replace(KeeperConstants.DS_NAME_PLACEHOLDER, dsName);
    ...
    public String getGlobalFullPullTopo() throws Exception {
        for (Topology topology : stormTopoHelper.topologySummary()) {
            String id = topology.getId();
            if (id.startsWith("global-splitter-puller")) {
                return id;
            }
        }
        return "";
    ...
    public Integer batchSendControMessage(String reloadType) {
        ...
        if (ROUTER_RELOAD.equalsIgnoreCase(reloadType)) {
            ...
            List<Map<String, Object>> payload = projectTableService.queryTopologyNames(null).getPayload(new TypeReference<List<Map<String, Object>>>() {
            });
            payload.forEach(topo -> {
                map.put("topic", topo.get("topoName") + "_ctrl");
                sendCtrlMessage(map);
            });
            return 0;
        }
    ...
    public List<HashMap<String, String>> checkCanal() throws Exception {
        //canal状态
        return autoDeployDataLineService.getCanalStatus(this.getDataSourceByDsType("mysql"));
    }
