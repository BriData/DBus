...
public class InitialLoadProcessor extends AbstractProcessor {
    ...
    public void process(final DBusConsumerRecord<String, byte[]> consumerRecord, Object... args) {
        ...
        try {
            ...
            //db2的处理方式与mysql和oracle不一样，db2通过schema registry获取schema信息
            String schemaName = Utils.buildAvroSchemaName(data.getSchemaId(), data.getTableName());
            Schema schema = SchemaProvider.getInstance().getSchemaBySchemaRegistry(schemaName, data.getPayload());
            ...
            String optype = record.get(Constants.DB2MessageBodyKey.DB2_ENTTYP).toString();
            //TODO 暂时放弃拉全量表的update/delete等消息
            if (!Constants.DB2MessageBodyKey.PT.equalsIgnoreCase(optype)) {
                listener.reduceFlowSize(consumerRecord.serializedValueSize());
                consumerListener.syncOffset(consumerRecord);
                return;
            }
...
//            }
            ...
            ControlMessage message = ControlMessageEncoder.DB2FullDataPollMessage(record, listener.getListenerId(), consumerRecord);
