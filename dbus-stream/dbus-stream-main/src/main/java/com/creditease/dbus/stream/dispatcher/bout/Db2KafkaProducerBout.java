...
public class Db2KafkaProducerBout extends BaseRichBolt {
        ...
        try {
            ...
            Callback callback = new Callback() {
                ...
                public void onCompletion(RecordMetadata ignored, Exception e) {
                    ...
                    if (e != null) {
                        logger.error(String.format("Send FAIL: toTopic=%s", toTopic));
                    } else {
                        logger.debug(String.format("  Send successful:  toTopic=%s, key=(%s)", toTopic, key));

                    }
                }
            };

            logger.info(String.format("  Sending: toTopic=%s, key=(%s), msgCount=%d", toTopic, key, msgCount));
...
//            logger.error(String.format("FAIL! Kafka bolt fails at offset (%s).", currentOffset.toString()));
            logger.error("FAIL! Kafka bolt failed!." + ex);
