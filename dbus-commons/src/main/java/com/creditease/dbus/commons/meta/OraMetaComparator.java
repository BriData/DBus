 ...
 */
public class OraMetaComparator extends AbstractMetaComparator {

    @Override
    protected boolean isCellSupported(MetaWrapper.MetaCell cell) {
        return SupportedOraDataType.isSupported(cell.getDataType()) && !cell.isHidden() && !cell.isVirtual();
    }

    @Override
    protected DataType convert(MetaWrapper.MetaCell cell) {
        return DataType.convert(cell.getDataType(), cell.getDataPrecision(), cell.getDataScale());
    }
}
