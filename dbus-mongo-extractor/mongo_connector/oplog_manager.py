...
from doc_managers import global_variable
...
class OplogThread(threading.Thread):
    ...
    ###############################################################################
    # DBUS: ximei ： 没有用上  2018.03.19
    # # update newest timestamp and if have failed message, then get new cursor
    # def _update_message_queue(self):
    #     with self.message_status_queue as message_queue:
    #         commit_element = message_queue.commit_point()
    #         update_timestamp = commit_element.key
    #         if update_timestamp is not None:
    #             print "*********  update_timestamp is: " + str(update_timestamp)
    #             self.update_checkpoint(util.long_to_bson_ts(update_timestamp))
    #             message_queue.pop_ok_elements()
    #         seek_element = message_queue.seek_point()
    #         if seek_element is not None:
    #             # when have failed element, then get new cursor
    #             # cursor = self.get_oplog_cursor(seek_element.key)
    #             return False
    ...
    def run(self):
        ...
        while self.running is True:
            # 妲珅添加reload实现
            LOG.debug("OplogThread: Getting need_reload")
            need_reload = retry_until_ok(self.init_needreload)
            LOG.critical("111 need_reload: " + str(need_reload))
            if need_reload:
                print "do reload event"
                for docman in self.doc_managers:
                    docman.load_schema_tables()

            # 妲珅添加礼貌退出功能
            LOG.debug("OplogThread: Getting thread_alive")
            thread_alive = global_variable.thread_alive
            if not thread_alive:
                for docman in self.doc_managers:
                    docman.stop()
                self.running = False
            ...
            try:
                ...
                while cursor.alive and self.running:
                    ...
                    for n, entry in enumerate(cursor):
                        ...
                        #############################################################################
                        # 妲珅：判断放的地方不对，应该放到外层循环
                        # DBUS: 2 oplog thread and kafka_ctrl thread exit, Politely exit
                        # docman_alive_flag = True
                        # for docman in self.doc_managers:
                        #     if not global_variable.thread_alive:
                        #         docman.stop()
                        #         docman_alive_flag = False
                        #
                        # if not docman_alive_flag:
                        #     break
                        #############################################################################
                        ...
                        for docman in self.doc_managers:
                            try:
                                ...
                                # Remove
                                if operation == 'd':
                                    # DBUS: docman.remove(entry['o']['_id'], ns, timestamp)
                                    docman.remove(entry, ns, timestamp)
                                    remove_inc += 1
                                ...
                                # Insert
                                    ...
                                    if is_gridfs_file:
                                        ...
                                        '''
                                        LOG.info("this is gridfs, and we don't support gridfs")
                                    else:
                                        # DBUS: docman.upsert(doc, ns, timestamp)
                                        docman.insert(entry, ns, timestamp)
                                    insert_inc += 1

                                # Update
                                elif operation == 'u':
                                    # DBUS: docman.update(entry['o2']['_id'], entry['o'], ns, timestamp)
                                    docman.update(entry, ns, timestamp)
                                    update_inc += 1

                                # Command
                                elif operation == 'c':
                                    # use unmapped namespace
                                    # DBUS:  doc = entry.get('o')
                                    # DBUS:  docman.handle_command(doc, entry['ns'], timestamp)
                                    docman.handle_command(entry, entry['ns'], timestamp)
                        ...
                        if (remove_inc + insert_inc + update_inc) % 1000 == 0:
                            LOG.debug(
                                "OplogThread: Documents removed: %d, "
                                "inserted: %d, updated: %d so far" % (
                                    remove_inc, insert_inc, update_inc))
                        ...
                        # DBUS:  async commit checkpoint
                        if (remove_inc + insert_inc + update_inc) % self.batch_size == 0:
                            # last_ts = None

                            ##################################################
                            #  search send status dict to find the last timestamp in the dict
                            with self.message_status_queue as message_queue:
                                LOG.debug("*********  message_status_queue size: %d " % message_queue.size())
                                commit_element = message_queue.commit_point()
                                if commit_element is not None:
                                    update_timestamp = commit_element.key
                                    LOG.debug("*********  update_timestamp is: " + str(update_timestamp))
                                    self.update_checkpoint(util.long_to_bson_ts(update_timestamp))
                                    message_queue.pop_ok_elements()

                                failed_element = message_queue.seek_failed_point()
                                if failed_element is not None:
                                    self.update_checkpoint(util.long_to_bson_ts(update_timestamp))
                                    break

                                if message_queue.size() > 1000:
                                    LOG.debug("*********  n %d sleep" % n)
                                    time.sleep(2)
                            ####################################################################
                    ...
                    with self.message_status_queue as message_queue:
                        LOG.debug("*********  message_status_queue size: %d " % message_queue.size())
                        commit_element = message_queue.commit_point()
                        # checking whether commit_elment is null
                        if commit_element is not None:
                            update_timestamp = commit_element.key
                            # if update_timestamp is not None:
                            LOG.debug("*********  update_timestamp is: " + str(update_timestamp))
    ...
    def init_needreload(self):
        need_reload = global_variable.need_reload
        return need_reload
    ...
    def init_cursor(self):
        ...
        if timestamp is None:
            ...
            if self.collection_dump:
                ...
                if timestamp is None:
                    return None, True
                '''
                LOG.info("we don't support dump collection")
            else:
        ...
        if cursor_empty:
            ...
            LOG.debug("OplogThread: Initiating rollback from "
                      "get_oplog_cursor")
            # DBUS: do not support rollback sink
